    ...
    def processPayment(self, user_id: str, amount: float) -> TransactionResult:
        ...
        for attempt in range(3):
            try:
                ...
                # Logowanie sukcesu
                logger.info(
                    f"Payment successful for user {user_id} with amount {amount}. Transaction ID: {result.transaction_id}")
                return result
                ...
                if attempt < 2:
                    ...
                    logger.error(
                        f"Network error during payment for user {user_id}. Error: {e}")
                    return TransactionResult(success=False, transaction_id="", message=str(e))
            except PaymentException as e:
                # Logowanie błędu płatności
                logger.error(
                    f"Payment failed for user {user_id} due to payment exception. Error: {e}")
                return TransactionResult(success=False, transaction_id="", message=str(e))
    ...
    def refundPayment(self, transaction_id: str) -> TransactionResult:
        ...
        for attempt in range(3):
            try:
                ...
                logger.info(
                    f"Refund successful for transaction ID {transaction_id}.")
                return result
            except NetworkException as e:
                if attempt < 2:
                    time.sleep(1)
                    ...
                    # Logowanie błędu sieci
                    logger.error(
                        f"Network error during refund for transaction ID {transaction_id}. Error: {e}")
                ...
                logger.error(
                    f"Refund failed for transaction ID {transaction_id} due to refund exception. Error: {e}")
                return TransactionResult(success=False, transaction_id=transaction_id, message=str(e))
    ...
    def getPaymentStatus(self, transaction_id: str) -> TransactionStatus:
        ...
        for attempt in range(3):
            try:
                ...
                logger.info(
                    f"Status retrieval successful for transaction ID {transaction_id}: {status.name}")
                return status
            except NetworkException as e:
                if attempt < 2:
                    time.sleep(1)
                    ...
                    logger.error(
                        f"Network error during status retrieval for transaction ID {transaction_id}. Error: {e}")
